...
namespace GrpcServiceMock.Services
{
    public class ProductService: ProductServiceBase
    {
        ...
        public ProductService(IRepository<Product> repositoryProduct, IRepository<Category> repositoryCategory)
        {
        ...
        {
            NTQTRAININGContext ntq = new NTQTRAININGContext();
            ...
            var pagingProduct = _repositoryProduct.GetAll().Skip((request.PageIndex - 1) * request.PageSize)
                .Take(request.PageSize).Select(x => new ProductProto()
                {
                    ...
                    Category = new CategoryProductProto
                    {
                        Name = ntq.Categories.SingleOrDefault(i => i.Id == x.CategoryId).Name
                    } 
                }) ;
        ...
        private CategoryProductProto MapDataToProtoCategory(int request)
        {
            var categoryProto = _repositoryCategory.GetById(request);
            var categoryProductProto = new CategoryProductProto();

            categoryProductProto.Id = categoryProto.Id;
            categoryProductProto.Name = categoryProto.Name;

            categoryProductProto.TagName = categoryProto.TagName;
            return categoryProductProto;
        }
    }
}
