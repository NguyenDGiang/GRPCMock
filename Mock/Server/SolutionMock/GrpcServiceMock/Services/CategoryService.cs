...
namespace GrpcServiceMock.Services
{
    public class CategoryService: CatagoryServiceBase
    {
        ...
        public CategoryService(IRepository<Category> repository, IRepository<Product> repositoryProduct)
        {
        ...
        {
            using (var contextDb = new NTQTRAININGContext())
            {
                using (IDbContextTransaction transaction = contextDb.Database.BeginTransaction())
                {
                    try
                    {
                        var categoryIdInProduct = contextDb.Products.FirstOrDefault(x => x.Id == request.Id);
                        if (categoryIdInProduct != null)
                        {
                            return Task.FromResult<CategoryResponse>(new CategoryResponse()
                            {
                                Data = null,
                                Status = $"Đang Dùng Không xóa được"
                            });
